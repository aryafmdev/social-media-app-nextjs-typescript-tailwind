...
export default function MyFollowingPage() {
  ...
  const following = useQuery({
    queryKey: ['me', 'following', 1, 20],
    queryFn: () => getMyFollowing(effectiveToken as string, 1, 20),
  ...
  const followMut = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['me', 'following', 1, 20] });
    },
    onError: () => {
      qc.invalidateQueries({ queryKey: ['me'] });
      qc.invalidateQueries({ queryKey: ['me', 'header'] });
  ...
  const list = (following.data?.items ?? []) as {
    username: string;
    name?: string;
    isFollowedByMe?: boolean;
    avatarUrl?: string;
