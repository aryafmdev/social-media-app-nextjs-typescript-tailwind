...
import { getFollowers, getFollowing } from '../../../../lib/api/follow';
...
export default function FollowersPage() {
  ...
  const res = useQuery({
    queryKey: ['followers', username, 1, 20, token ? 'auth' : 'anon'],
    queryFn: () => getFollowers(username as string, 1, 20, token ?? undefined),
    enabled: !!username,
  ...
  const myFollowing = useQuery({
    ...
    queryFn: () =>
      getFollowing(myUsername as string, 1, 200, token ?? undefined),
    enabled: !!token && !!myUsername,
    staleTime: 0,
    refetchOnMount: true,
  ...
  const followMut = useMutation({
    ...
    onMutate: async ({ uname, next }) => {
      ...
      }>(['followers', username, 1, 20, token ? 'auth' : 'anon'], (prev) => {
                  ...
                  >
                    ...
                    {isMe ? (
                      <button
                        ...
                        aria-label={`me-following-owner-${u.username}`}
                        disabled
                      >
